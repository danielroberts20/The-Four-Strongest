...
rightmost = pulp.LpVariable("rightmost", lowBound=0)
...
# 4. Overlap constraints based on orientation-dependent lengths (with tolerance)
for i, i2 in permutations(TOYS, 2):
    for k in POSITIONS[:-1]:  # positions 0, 1, 2
        # Only apply when i is in position k and i2 in k+1
        model += (
            s[i] + pulp.lpSum(b[(i, j)] * j_i[i, j] for j in ORIENTATIONS)
            <= s[i2] + TOLERANCE + M * (2 - x[i, k] - x[i2, k + 1])
        )
# 5.
for i in TOYS:
    model += s[i] + pulp.lpSum(b[(i, j)] * j_i[i, j] for j in ORIENTATIONS) <= rightmost
